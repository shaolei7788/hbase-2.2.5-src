...
class ConnectionImplementation implements ClusterConnection, Closeable {
    ...
    ConnectionImplementation(Configuration conf, ExecutorService pool, User user) throws IOException {
        ...
        if(conf.getBoolean(NonceGenerator.CLIENT_NONCES_ENABLED_KEY, true)) {
            synchronized(nonceGeneratorCreateLock) {
                if(nonceGenerator == null) {
                    ...
                    /********
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *   注释：每个客户端随机的 NonceGEnerator,主要是为了生成 clientid
                     */
        ...
        // TODO_MA 注释：创建跟踪该connection所相关的region 信息监控实例
        this.stats = ServerStatisticTracker.create(conf);

        // TODO_MA 注释：远程服务器出现故障时,进行处理的机制
        this.interceptor = (new RetryingCallerInterceptorFactory(conf)).build();

        // TODO_MA 注释：RpcRetryingCaller创建工厂
        this.rpcControllerFactory = RpcControllerFactory.instantiate(conf);
        this.rpcCallerFactory = RpcRetryingCallerFactory.instantiate(conf, interceptor, this.stats);

        // TODO_MA 注释：create the backoff policy
        ...
        Class<? extends ClusterStatusListener.Listener> listenerClass = conf
                .getClass(ClusterStatusListener.STATUS_LISTENER_CLASS, ClusterStatusListener.DEFAULT_STATUS_LISTENER_CLASS,
        ...
        try {
            ...
            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释：用于获取集群的基本信息例如 clusterid 以及 region location 的 meta 数据
             *   默认实现是 ZKAsyncRegistry
             */
            ...
            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释： 获取一个 RpcClient = NettyRpcClient, 负责IPC调用相关
             */
            ...
            if(shouldListen) {
                if(listenerClass == null) {
                    LOG.warn(
                            HConstants.STATUS_PUBLISHED + " is true, but " + ClusterStatusListener.STATUS_LISTENER_CLASS + " is not set - not listening status");
                } else {
    ...
    public TableBuilder getTableBuilder(TableName tableName, ExecutorService pool) {
        ...
        return new TableBuilderBase(tableName, connectionConfig) {
            ...
            public Table build() {
                ...
                /********
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *   注释： 自己调用 HTable 构造器来构建实例对象的
                 */
                return new HTable(ConnectionImplementation.this, this, rpcCallerFactory, rpcControllerFactory, pool);
            }
        };
    }
    ...
    public BufferedMutator getBufferedMutator(BufferedMutatorParams params) {
        ...
        if(params.getWriteBufferPeriodicFlushTimeoutMs() == BufferedMutatorParams.UNSET) {
            params.setWriteBufferPeriodicFlushTimeoutMs(connectionConfig.getWriteBufferPeriodicFlushTimeoutMs());
        }
        if(params.getWriteBufferPeriodicFlushTimerTickMs() == BufferedMutatorParams.UNSET) {
            params.setWriteBufferPeriodicFlushTimerTickMs(connectionConfig.getWriteBufferPeriodicFlushTimerTickMs());
        }
        ...
        try {
            return (BufferedMutator) ReflectionUtils
                    .newInstance(Class.forName(implementationClassName), this, rpcCallerFactory, rpcControllerFactory, params);
    ...
    public Hbck getHbck(ServerName masterServer) throws IOException {
        ...
        String key = getStubKey(MasterProtos.HbckService.BlockingInterface.class.getName(), masterServer, this.hostnamesCanChange);

        return new HBaseHbck((MasterProtos.HbckService.BlockingInterface) computeIfAbsentEx(stubs, key, () -> {
            BlockingRpcChannel channel = this.rpcClient.createBlockingRpcChannel(masterServer, user, rpcTimeout);
        ...
        if(this.metaLookupPool == null) {
            synchronized(this) {
                if(this.metaLookupPool == null) {
                    ...
                    this.metaLookupPool = getThreadPool(threads, threads, "-metaLookup-shared-", new LinkedBlockingQueue<>());
                }
            }
        }
        ...
        try {
            ...
            for(Pair<RegionInfo, ServerName> pair : locations) {
                ...
                if(pair.getSecond() == null) {
                    LOG.debug("Table {} has not deployed region {}", tableName, pair.getFirst().getEncodedName());
                    notDeployed++;
            ...
            if(notDeployed > 0) {
                ...
                if(LOG.isDebugEnabled()) {
                    LOG.debug("Table {} expected to have {} regions, but only {} available", tableName, splitKeys.length + 1, regionCount);
                }
                return false;
            } else {
    ...
    public HRegionLocation locateRegion(final byte[] regionName) throws IOException {
        RegionLocations locations = locateRegion(RegionInfo.getTable(regionName), RegionInfo.getStartKey(regionName), false, true);
    ...
    public RegionLocations locateRegion(final TableName tableName, final byte[] row, boolean useCache, boolean retry,
        ...
        if(tableName.equals(TableName.META_TABLE_NAME)) {
            ...
            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释： 如果是 hbase:meta 表，则发请求给 zookeeper
             */
            ...
            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释： 如果是普通表，则发请求给 meta 所在的 regionserver
             */
    ...
    private RegionLocations locateMeta(final TableName tableName, boolean useCache, int replicaId) throws IOException {
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 如果开启了 meta 缓存，则直接从缓存中获取 meta 表的为止即可。
         */
        if(useCache) {
        ...
        synchronized(metaRegionLock) {
            ...
            // TODO_MA 注释：再来一次的目的，就是怕这个客户端中的其他线程，获取了 meta 的为止，已经放入到缓存中了。
            ...
            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释： 定位 zookeeper 访问 meta 表所在的位置
    ...
    private RegionLocations locateRegionInMeta(TableName tableName, byte[] row, boolean useCache, boolean retry, int replicaId) throws IOException {
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 现在缓存中，定位 region 的信息。
         */
        // If we are supposed to be using the cache, look in the cache to see if we already have the region.
        if(useCache) {
        ...
        // the extra 9's on the end are necessary to allow "exact" matches without knowing the precise region names.
        ...
        Scan s = new Scan().withStartRow(metaStartKey).withStopRow(metaStopKey, true).addFamily(HConstants.CATALOG_FAMILY).setReversed(true)
                .setCaching(5).setReadType(ReadType.PREAD);
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释：maxAttempts 最大重试次数，由参数hbase.client.retries.number指定，默认为15次
        ...
        for(int tries = 0; ; tries++) {
            ...
            if(tries >= maxAttempts) {
                throw new NoServerForRegionException(
                        "Unable to find region for " + Bytes.toStringBinary(row) + " in " + tableName + " after " + tries + " tries.");
            }
            ...
            if(useCache) {
                ...
                // TODO_MA 注释：如果不需要使用缓存，删除任何现有的缓存，以免干扰。
                // If we are not supposed to be using the cache, delete any existing cached location so it won't interfere.
            ...
            try {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *   注释： 扫描 meta 表
                 *  ReversedClientScanner rcs = new ReversedClientScanner()
                 *  rcs.next();
                 */
                try(ReversedClientScanner rcs = new ReversedClientScanner(conf, s, TableName.META_TABLE_NAME, this, rpcCallerFactory,
                        rpcControllerFactory, getMetaLookupPool(), metaReplicaCallTimeoutScanInMicroSecond)) {
                    boolean tableNotFound = true;
                    for(; ; ) {

                        // TODO_MA 注释：获取 region 的位置
                        ...
                        /********
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *   注释：把结果转换为我们需要的 HRegionLocation
                         *   locations 就是我们要插入的数据的 region 的位置信息
                         */
                        ...
                        // TODO_MA 注释：即使children在线，我们也可能找到分离的父region，所以在这里我们需要跳过这个region并转到下一个region。
                        ...
                        // TODO_MA 注释：分裂的孩子可能还没有在线，我们在上述情况下跳过了父region，所以我们可能已经到达了一个不包含我们的region。
                        ...
                        // TODO_MA 注释：做 serverName 校验
                        ServerName serverName = locations.getRegionLocation(replicaId).getServerName();
                        if(serverName == null) {
                            throw new NoServerForRegionException(
                                    "No server address listed in " + TableName.META_TABLE_NAME + " for region " + regionInfo
                                            .getRegionNameAsString() + " containing row " + Bytes.toStringBinary(row));
                        }
                        if(isDeadServer(serverName)) {
                            throw new RegionServerStoppedException("hbase:meta says the region " + regionInfo
                                    .getRegionNameAsString() + " is managed by the server " + serverName + ", but it is dead.");
                        }

                        /********
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *   注释： 把该表的 region 位置信息，缓存在 metaCache 中
                         */
                ...
                if(tries < maxAttempts - 1) {
                    LOG.debug("locateRegionInMeta parentTable='{}', attempt={} of {} failed; retrying " + "after sleep of {}",
    ...
    static class ServerErrorTracker {
        ...
        boolean canTryMore(int numAttempt) {
            ...
            return numAttempt < maxTries || (maxTries > 1 && EnvironmentEdgeManager.currentTime() < this.canRetryUntil);
        }
    ...
    private final class MasterServiceStubMaker {
        ...
        private MasterProtos.MasterService.BlockingInterface makeStubNoRetries() throws IOException, KeeperException {
            ...
            String key = getStubKey(MasterProtos.MasterService.getDescriptor().getName(), sn, hostnamesCanChange);
            MasterProtos.MasterService.BlockingInterface stub = (MasterProtos.MasterService.BlockingInterface) computeIfAbsentEx(stubs, key, () -> {
    ...
    public BlockingInterface getClient(ServerName serverName) throws IOException {
        ...
        String key = getStubKey(ClientProtos.ClientService.BlockingInterface.class.getName(), serverName, this.hostnamesCanChange);
        return (ClientProtos.ClientService.BlockingInterface) computeIfAbsentEx(stubs, key, () -> {

            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释：rpcClient = NettyRpcClient， 是  AbstractRpcClient 的子类
             */
            BlockingRpcChannel channel = this.rpcClient.createBlockingRpcChannel(serverName, user, rpcTimeout);
    ...
    public MasterKeepAliveConnection getMaster() throws IOException {
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： master = getKeepAliveMasterService()
         */
    ...
    private MasterKeepAliveConnection getKeepAliveMasterService() throws IOException {
        ...
        return new MasterKeepAliveConnection() {
            ...
            @Override
            public MasterProtos.IsInMaintenanceModeResponse isMasterInMaintenanceMode(final RpcController controller,
            ...
            @Override
            public MasterProtos.IsCatalogJanitorEnabledResponse isCatalogJanitorEnabled(RpcController controller,
                    MasterProtos.IsCatalogJanitorEnabledRequest request) throws ServiceException {
            ...
            @Override
            public MasterProtos.SetCleanerChoreRunningResponse setCleanerChoreRunning(RpcController controller,
                    MasterProtos.SetCleanerChoreRunningRequest request) throws ServiceException {
            ...
            @Override
            public MasterProtos.ListTableDescriptorsByNamespaceResponse listTableDescriptorsByNamespace(RpcController controller,
                    MasterProtos.ListTableDescriptorsByNamespaceRequest request) throws ServiceException {
                return stub.listTableDescriptorsByNamespace(controller, request);
            }

            @Override
            public MasterProtos.ListTableNamesByNamespaceResponse listTableNamesByNamespace(RpcController controller,
                    MasterProtos.ListTableNamesByNamespaceRequest request) throws ServiceException {
            ...
            @Override
            public MasterProtos.MajorCompactionTimestampResponse getLastMajorCompactionTimestamp(RpcController controller,
                    MasterProtos.MajorCompactionTimestampRequest request) throws ServiceException {
                return stub.getLastMajorCompactionTimestamp(controller, request);
            }

            @Override
            public MasterProtos.MajorCompactionTimestampResponse getLastMajorCompactionTimestampForRegion(RpcController controller,
                    MasterProtos.MajorCompactionTimestampForRegionRequest request) throws ServiceException {
            ...
            @Override
            public MasterProtos.SetSplitOrMergeEnabledResponse setSplitOrMergeEnabled(RpcController controller,
    ...
    public void updateCachedLocations(final TableName tableName, byte[] regionName, byte[] rowkey, final Object exception, final ServerName source) {
        if(rowkey == null || tableName == null) {
            LOG.warn(
                    "Coding error, see method javadoc. row=" + (rowkey == null ? "null" : rowkey) + ", tableName=" + (tableName == null ? "null" : tableName));
            return;
        }
        ...
        if(cause != null) {
            ...
            if(cause instanceof RegionMovedException) {
                ...
                if(LOG.isTraceEnabled()) {
                    LOG.trace("Region " + regionInfo.getRegionNameAsString() + " moved to " + rme.getHostname() + ":" + rme
                            .getPort() + " according to " + source.getAddress());
                }
                 ...
                 *   注释：
                 */
                // We know that the region is not anymore on this region server, but we know the new location.
