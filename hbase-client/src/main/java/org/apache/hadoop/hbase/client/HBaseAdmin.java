...
public class HBaseAdmin implements Admin {
    ...
    HBaseAdmin(ClusterConnection connection) throws IOException {
        ...
        this.getProcedureTimeout = this.conf.getInt("hbase.client.procedure.future.get.timeout.msec", 10 * 60000); // 10min
    ...
    public boolean abortProcedure(final long procId, final boolean mayInterruptIfRunning) throws IOException {
        return get(abortProcedureAsync(procId, mayInterruptIfRunning), this.syncWaitTimeout, TimeUnit.MILLISECONDS);
    }

    @Override
    public Future<Boolean> abortProcedureAsync(final long procId, final boolean mayInterruptIfRunning) throws IOException {
    ...
    public List<TableDescriptor> listTableDescriptors(Pattern pattern, boolean includeSysTables) throws IOException {
        return executeCallable(new MasterCallable<List<TableDescriptor>>(getConnection(), getRpcControllerFactory()) {
            @Override
    ...
    public TableDescriptor getDescriptor(TableName tableName) throws TableNotFoundException, IOException {
         ...
         *   注释：
         */
        return getTableDescriptor(tableName, getConnection(), rpcCallerFactory, rpcControllerFactory, operationTimeout, rpcTimeout);
    }
    ...
    public Future<Void> modifyTableAsync(TableDescriptor td) throws IOException {
        ModifyTableResponse response = executeCallable(new MasterCallable<ModifyTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected ModifyTableResponse rpcCall() throws Exception {
                ...
                ModifyTableRequest request = RequestConverter.buildModifyTableRequest(td.getTableName(), td, nonceGroup, nonce);
        ...
        return executeCallable(new MasterCallable<List<TableDescriptor>>(getConnection(), getRpcControllerFactory()) {
            ...
            protected List<TableDescriptor> rpcCall() throws Exception {
                return master.listTableDescriptorsByNamespace(getRpcController(),
                        ListTableDescriptorsByNamespaceRequest.newBuilder().setNamespaceName(Bytes.toString(name)).build()).getTableSchemaList()
                        .stream().map(ProtobufUtil::toTableDescriptor).collect(Collectors.toList());
            }
        });
    }
    ...
    private static class AbortProcedureFuture extends ProcedureFuture<Boolean> {
        ...
        public AbortProcedureFuture(final HBaseAdmin admin, final Long procId, final Boolean abortProcResponse) {
            super(admin, procId);
    ...
    public HTableDescriptor[] listTables(final Pattern pattern, final boolean includeSysTables) throws IOException {
        return executeCallable(new MasterCallable<HTableDescriptor[]>(getConnection(), getRpcControllerFactory()) {
            ...
            protected HTableDescriptor[] rpcCall() throws Exception {
                GetTableDescriptorsRequest req = RequestConverter.buildGetTableDescriptorsRequest(pattern, includeSysTables);
                return ProtobufUtil.toTableDescriptorList(master.getTableDescriptors(getRpcController(), req)).stream()
    ...
    @Override
    public TableName[] listTableNames(final String regex, final boolean includeSysTables) throws IOException {
    ...
    public HTableDescriptor getTableDescriptor(final TableName tableName) throws IOException {
        return getHTableDescriptor(tableName, getConnection(), rpcCallerFactory, rpcControllerFactory, operationTimeout, rpcTimeout);
    }

    static TableDescriptor getTableDescriptor(final TableName tableName, Connection connection, RpcRetryingCallerFactory rpcCallerFactory,
            final RpcControllerFactory rpcControllerFactory, int operationTimeout, int rpcTimeout) throws IOException {
        if(tableName == null)
            return null;
         ...
         *   注释：
         */
        TableDescriptor td = executeCallable(new MasterCallable<TableDescriptor>(connection, rpcControllerFactory) {
            @Override
     ...
     */
    @Deprecated
    static HTableDescriptor getHTableDescriptor(final TableName tableName, Connection connection, RpcRetryingCallerFactory rpcCallerFactory,
            final RpcControllerFactory rpcControllerFactory, int operationTimeout, int rpcTimeout) throws IOException {
        if(tableName == null) {
            return null;
        }
    ...
    @Override
    public void createTable(final TableDescriptor desc, byte[][] splitKeys) throws IOException {

        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释：
         *   1、createTableAsync 是一个异步创建 Table 的方法实现
         *   2、get 就是获取创建结果
         */
    ...
    public Future<Void> createTableAsync(final TableDescriptor desc, final byte[][] splitKeys) throws IOException {
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 关于 HBase 创建时预分区的 splits 的排序和去重操作
         */
        if(splitKeys != null && splitKeys.length > 0) {

            // TODO_MA 注释：预分区的 splitKeys 排序
            ...
            for(byte[] splitKey : splitKeys) {
                ...
                // TODO_MA 注释：如果 splitKey 有等于 0  证明是无效的，报错
                if(Bytes.compareTo(splitKey, HConstants.EMPTY_BYTE_ARRAY) == 0) {
                    throw new IllegalArgumentException("Empty split key must not be passed in the split keys.");
                }

                // TODO_MA 注释：如果发现 splitKey 有相同的，则报错
                if(lastKey != null && Bytes.equals(splitKey, lastKey)) {
                    throw new IllegalArgumentException(
                            "All split keys must be unique, " + "found duplicate: " + Bytes.toStringBinary(splitKey) + ", " + Bytes
        ...
        CreateTableResponse response = executeCallable(new MasterCallable<CreateTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected CreateTableResponse rpcCall() throws Exception {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *   注释：
                 *   1、构建 CreateTableRequest
                 *   2、发送请求 CreateTableRequest 到 Master
                 *   3、服务跳转到 MasterService
                 */
                CreateTableRequest request = RequestConverter.buildCreateTableRequest(desc, splitKeys, nonceGroup, nonce);
    ...
    private static class CreateTableFuture extends TableFuture<Void> {
        ...
        public CreateTableFuture(final HBaseAdmin admin, final TableDescriptor desc, final byte[][] splitKeys, final CreateTableResponse response) {
            super(admin, desc.getTableName(), (response != null && response.hasProcId()) ? response.getProcId() : null);
    ...
    public Future<Void> deleteTableAsync(final TableName tableName) throws IOException {
        DeleteTableResponse response = executeCallable(new MasterCallable<DeleteTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected DeleteTableResponse rpcCall() throws Exception {
                ...
                DeleteTableRequest req = RequestConverter.buildDeleteTableRequest(tableName, nonceGroup, nonce);
    ...
    private static class DeleteTableFuture extends TableFuture<Void> {
        public DeleteTableFuture(final HBaseAdmin admin, final TableName tableName, final DeleteTableResponse response) {
    ...
    public Future<Void> truncateTableAsync(final TableName tableName, final boolean preserveSplits) throws IOException {
        TruncateTableResponse response = executeCallable(new MasterCallable<TruncateTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected TruncateTableResponse rpcCall() throws Exception {
                ...
                TruncateTableRequest req = RequestConverter.buildTruncateTableRequest(tableName, preserveSplits, nonceGroup, nonce);
    ...
    private static class TruncateTableFuture extends TableFuture<Void> {
        ...
        public TruncateTableFuture(final HBaseAdmin admin, final TableName tableName, final boolean preserveSplits,
    ...
    public Future<Void> enableTableAsync(final TableName tableName) throws IOException {
        ...
        EnableTableResponse response = executeCallable(new MasterCallable<EnableTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected EnableTableResponse rpcCall() throws Exception {
                ...
                EnableTableRequest req = RequestConverter.buildEnableTableRequest(tableName, nonceGroup, nonce);
    ...
    private static class EnableTableFuture extends TableFuture<Void> {
        public EnableTableFuture(final HBaseAdmin admin, final TableName tableName, final EnableTableResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    public Future<Void> disableTableAsync(final TableName tableName) throws IOException {
        ...
        DisableTableResponse response = executeCallable(new MasterCallable<DisableTableResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected DisableTableResponse rpcCall() throws Exception {
                ...
                DisableTableRequest req = RequestConverter.buildDisableTableRequest(tableName, nonceGroup, nonce);
    ...
    private static class DisableTableFuture extends TableFuture<Void> {
        public DisableTableFuture(final HBaseAdmin admin, final TableName tableName, final DisableTableResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    @Override
    public void addColumnFamily(final TableName tableName, final ColumnFamilyDescriptor columnFamily) throws IOException {
    ...
    public Future<Void> addColumnFamilyAsync(final TableName tableName, final ColumnFamilyDescriptor columnFamily) throws IOException {
        AddColumnResponse response = executeCallable(new MasterCallable<AddColumnResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected AddColumnResponse rpcCall() throws Exception {
                setPriority(tableName);
                AddColumnRequest req = RequestConverter.buildAddColumnRequest(tableName, columnFamily, nonceGroup, nonce);
    ...
    private static class AddColumnFamilyFuture extends ModifyTableFuture {
        public AddColumnFamilyFuture(final HBaseAdmin admin, final TableName tableName, final AddColumnResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    @Override
    @Deprecated
    public void deleteColumn(final TableName tableName, final byte[] columnFamily) throws IOException {
        deleteColumnFamily(tableName, columnFamily);
    }

    @Override
    public void deleteColumnFamily(final TableName tableName, final byte[] columnFamily) throws IOException {
    ...
    public Future<Void> deleteColumnFamilyAsync(final TableName tableName, final byte[] columnFamily) throws IOException {
        DeleteColumnResponse response = executeCallable(new MasterCallable<DeleteColumnResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected DeleteColumnResponse rpcCall() throws Exception {
                setPriority(tableName);
                DeleteColumnRequest req = RequestConverter.buildDeleteColumnRequest(tableName, columnFamily, nonceGroup, nonce);
    ...
    private static class DeleteColumnFamilyFuture extends ModifyTableFuture {
        public DeleteColumnFamilyFuture(final HBaseAdmin admin, final TableName tableName, final DeleteColumnResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    @Override
    public void modifyColumnFamily(final TableName tableName, final ColumnFamilyDescriptor columnFamily) throws IOException {
    ...
    public Future<Void> modifyColumnFamilyAsync(final TableName tableName, final ColumnFamilyDescriptor columnFamily) throws IOException {
        ModifyColumnResponse response = executeCallable(new MasterCallable<ModifyColumnResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected ModifyColumnResponse rpcCall() throws Exception {
                setPriority(tableName);
                ModifyColumnRequest req = RequestConverter.buildModifyColumnRequest(tableName, columnFamily, nonceGroup, nonce);
    ...
    private static class ModifyColumnFamilyFuture extends ModifyTableFuture {
        public ModifyColumnFamilyFuture(final HBaseAdmin admin, final TableName tableName, final ModifyColumnResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    @Deprecated
    @Override
    public boolean closeRegionWithEncodedRegionName(final String encodedRegionName, final String unused) throws IOException {
    ...
    private void flush(AdminService.BlockingInterface admin, final RegionInfo info) throws IOException {
        ProtobufUtil.call(() -> {
            // TODO: There is no timeout on this controller. Set one!
            HBaseRpcController controller = rpcControllerFactory.newController();
            FlushRegionRequest request = RequestConverter.buildFlushRegionRequest(info.getRegionName());
    ...
    private Boolean switchCompact(AdminService.BlockingInterface admin, boolean onOrOff) throws IOException {
        return executeCallable(new RpcRetryingCallable<Boolean>() {
            @Override
            protected Boolean rpcCall(int callTimeout) throws Exception {
                HBaseRpcController controller = rpcControllerFactory.newController();
                CompactionSwitchRequest request = CompactionSwitchRequest.newBuilder().setEnabled(onOrOff).build();
                CompactionSwitchResponse compactionSwitchResponse = admin.compactionSwitch(controller, request);
     ...
     */
    @Override
    public void majorCompact(final TableName tableName, final byte[] columnFamily) throws IOException {
        compact(tableName, columnFamily, true, CompactType.NORMAL);
    }

    @Override
    public void majorCompactRegion(final byte[] regionName, final byte[] columnFamily) throws IOException {
    ...
    private void compact(final TableName tableName, final byte[] columnFamily, final boolean major, CompactType compactType) throws IOException {
        switch(compactType) {
            case MOB:
                compact(this.connection.getAdminForMaster(), RegionInfo.createMobRegionInfo(tableName), major, columnFamily);
                break;
            case NORMAL:
                checkTableExists(tableName);
                for(HRegionLocation loc : connection.locateRegions(tableName, false, false)) {
                    ...
                    try {
                        ...
                        if(LOG.isDebugEnabled()) {
                            LOG.debug(
                                    "Trying to" + (major ? " major" : "") + " compact " + loc.getRegion() + ": " + StringUtils.stringifyException(e));
                        }
                    }
                }
                break;
     ...
     */
    private void compactRegion(final byte[] regionName, final byte[] columnFamily, final boolean major) throws IOException {
        Pair<RegionInfo, ServerName> regionServerPair = getRegion(regionName);
        if(regionServerPair == null) {
        ...
        compact(this.connection.getAdmin(regionServerPair.getSecond()), regionServerPair.getFirst(), major, columnFamily);
    }

    private void compact(AdminService.BlockingInterface admin, RegionInfo hri, boolean major, byte[] family) throws IOException {
    ...
    public boolean balance(final boolean force) throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                return master.balance(getRpcController(), RequestConverter.buildBalanceRequest(force)).getBalancerRan();
            }
        });
    }
    ...
    public CacheEvictionStats clearBlockCache(final TableName tableName) throws IOException {
        ...
        List<Pair<RegionInfo, ServerName>> pairs = MetaTableAccessor.getTableRegionsAndLocations(connection, tableName);
        Map<ServerName, List<RegionInfo>> regionInfoByServerName = pairs.stream().filter(pair -> !(pair.getFirst().isOffline()))
                .filter(pair -> pair.getSecond() != null)
                .collect(Collectors.groupingBy(pair -> pair.getSecond(), Collectors.mapping(pair -> pair.getFirst(), Collectors.toList())));
        ...
        for(Map.Entry<ServerName, List<RegionInfo>> entry : regionInfoByServerName.entrySet()) {
            ...
            if(stats.getExceptionCount() > 0) {
                for(Map.Entry<byte[], Throwable> exception : stats.getExceptions().entrySet()) {
                    LOG.debug("Failed to clear block cache for " + Bytes.toStringBinary(exception.getKey()) + " on " + entry.getKey() + ": ",
                            exception.getValue());
                }
            }
        }
    ...
    private CacheEvictionStats clearBlockCache(final ServerName sn, final List<RegionInfo> hris) throws IOException {
        HBaseRpcController controller = rpcControllerFactory.newController();
        AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
    ...
    public boolean runCleanerChore() throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            public Boolean rpcCall() throws Exception {
                return master.runCleanerChore(getRpcController(), RequestConverter.buildRunCleanerChoreRequest()).getCleanerChoreRan();
            }
        });
    }
    ...
    public boolean isCleanerChoreEnabled() throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            public Boolean rpcCall() throws Exception {
                return master.isCleanerChoreEnabled(getRpcController(), RequestConverter.buildIsCleanerChoreEnabledRequest()).getValue();
            }
        });
    }
     ...
     */
    @VisibleForTesting
    public void mergeRegionsSync(final byte[] nameOfRegionA, final byte[] nameOfRegionB, final boolean forcible) throws IOException {
        get(mergeRegionsAsync(nameOfRegionA, nameOfRegionB, forcible), syncWaitTimeout, TimeUnit.MILLISECONDS);
    }
    ...
    @Deprecated
    @Override
    public void mergeRegions(final byte[] nameOfRegionA, final byte[] nameOfRegionB, final boolean forcible) throws IOException {
    ...
    public Future<Void> mergeRegionsAsync(final byte[][] nameofRegionsToMerge, final boolean forcible) throws IOException {
        ...
        for(int i = 0; i < nameofRegionsToMerge.length; i++) {
            encodedNameofRegionsToMerge[i] = RegionInfo.isEncodedRegionName(nameofRegionsToMerge[i]) ? nameofRegionsToMerge[i] : Bytes
                    .toBytes(RegionInfo.encodeRegionName(nameofRegionsToMerge[i]));
        }
        ...
        for(int i = 0; i < nameofRegionsToMerge.length; i++) {
            ...
            if(pair != null) {
                ...
                if(tableName == null) {
                    ...
                    throw new IllegalArgumentException(
                            "Cannot merge regions from two different tables " + tableName + " and " + pair.getFirst().getTable());
                }
            } else {
    ...
    private static class MergeTableRegionsFuture extends TableFuture<Void> {
        public MergeTableRegionsFuture(final HBaseAdmin admin, final TableName tableName, final MergeTableRegionsResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }

        public MergeTableRegionsFuture(final HBaseAdmin admin, final TableName tableName, final Long procId) {
     ...
     * @throws IOException
     */
    public void splitRegionSync(byte[] regionName, byte[] splitPoint, final long timeout, final TimeUnit units) throws IOException {
    ...
    public Future<Void> splitRegionAsync(byte[] regionName, byte[] splitPoint) throws IOException {
        byte[] encodedNameofRegionToSplit = HRegionInfo.isEncodedRegionName(regionName) ? regionName : Bytes
                .toBytes(HRegionInfo.encodeRegionName(regionName));
        Pair<RegionInfo, ServerName> pair = getRegion(regionName);
        if(pair != null) {
            if(pair.getFirst() != null && pair.getFirst().getReplicaId() != HRegionInfo.DEFAULT_REPLICA_ID) {
    ...
    Future<Void> splitRegionAsync(RegionInfo hri, byte[] splitPoint) throws IOException {
        ...
        SplitTableRegionResponse response = executeCallable(new MasterCallable<SplitTableRegionResponse>(getConnection(), getRpcControllerFactory()) {
            Long nonceGroup = ng.getNonceGroup();
            Long nonce = ng.newNonce();
            ...
            protected SplitTableRegionResponse rpcCall() throws Exception {
                setPriority(tableName);
                SplitTableRegionRequest request = RequestConverter.buildSplitTableRegionRequest(hri, splitPoint, nonceGroup, nonce);
    ...
    private static class SplitTableRegionFuture extends TableFuture<Void> {
        public SplitTableRegionFuture(final HBaseAdmin admin, final TableName tableName, final SplitTableRegionResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }

        public SplitTableRegionFuture(final HBaseAdmin admin, final TableName tableName, final Long procId) {
            super(admin, tableName, procId);
        }
    ...
    public void split(final TableName tableName, final byte[] splitPoint) throws IOException {
        ...
        for(HRegionLocation loc : connection.locateRegions(tableName, false, false)) {
            ...
            if(r.getReplicaId() != RegionInfo.DEFAULT_REPLICA_ID || (splitPoint != null && !r.containsRow(splitPoint))) {
                continue;
            }
    ...
    @Override
    public void modifyTable(final TableName tableName, final TableDescriptor td) throws IOException {
        get(modifyTableAsync(tableName, td), syncWaitTimeout, TimeUnit.MILLISECONDS);
    }

    @Override
    public Future<Void> modifyTableAsync(final TableName tableName, final TableDescriptor td) throws IOException {
        if(!tableName.equals(td.getTableName())) {
            throw new IllegalArgumentException("the specified table name '" + tableName + "' doesn't match with the HTD one: " + td.getTableName());
        }
        return modifyTableAsync(td);
    }

    private static class ModifyTableFuture extends TableFuture<Void> {
        public ModifyTableFuture(final HBaseAdmin admin, final TableName tableName, final ModifyTableResponse response) {
            super(admin, tableName, (response != null && response.hasProcId()) ? response.getProcId() : null);
        }
    ...
    public ClusterMetrics getClusterMetrics(EnumSet<Option> options) throws IOException {
        return executeCallable(new MasterCallable<ClusterMetrics>(getConnection(), this.rpcControllerFactory) {
            @Override
            protected ClusterMetrics rpcCall() throws Exception {
                ...
                return ClusterMetricsBuilder.toClusterMetrics(master.getClusterStatus(getRpcController(), req).getClusterStatus());
            }
        });
    }
    ...
    public List<RegionMetrics> getRegionMetrics(ServerName serverName, TableName tableName) throws IOException {
        ...
        try {
            return admin.getRegionLoad(controller, request).getRegionLoadsList().stream().map(RegionMetricsBuilder::toRegionMetrics)
     ...
     */
    private static <T> T get(final Future<T> future, final long timeout, final TimeUnit units) throws IOException {
        try {
    ...
    public Future<Void> createNamespaceAsync(final NamespaceDescriptor descriptor) throws IOException {
        CreateNamespaceResponse response = executeCallable(new MasterCallable<CreateNamespaceResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected CreateNamespaceResponse rpcCall() throws Exception {
                return master.createNamespace(getRpcController(), CreateNamespaceRequest.newBuilder().setNamespaceDescriptor(ProtobufUtil.
    ...
    public NamespaceDescriptor getNamespaceDescriptor(final String name) throws NamespaceNotFoundException, IOException {
        return executeCallable(new MasterCallable<NamespaceDescriptor>(getConnection(), getRpcControllerFactory()) {
            ...
            protected NamespaceDescriptor rpcCall() throws Exception {
                return ProtobufUtil
                        .toNamespaceDescriptor(master.getNamespaceDescriptor(getRpcController(), GetNamespaceDescriptorRequest.newBuilder().
    ...
    public NamespaceDescriptor[] listNamespaceDescriptors() throws IOException {
        return executeCallable(new MasterCallable<NamespaceDescriptor[]>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected NamespaceDescriptor[] rpcCall() throws Exception {
                List<HBaseProtos.NamespaceDescriptor> list = master
                        .listNamespaceDescriptors(getRpcController(), ListNamespaceDescriptorsRequest.newBuilder().build())
    ...
    public HTableDescriptor[] listTableDescriptorsByNamespace(final String name) throws IOException {
        return executeCallable(new MasterCallable<HTableDescriptor[]>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected HTableDescriptor[] rpcCall() throws Exception {
                List<TableSchema> list = master.listTableDescriptorsByNamespace(getRpcController(),
                        ListTableDescriptorsByNamespaceRequest.newBuilder().setNamespaceName(name).build()).getTableSchemaList();
    ...
    public TableName[] listTableNamesByNamespace(final String name) throws IOException {
        return executeCallable(new MasterCallable<TableName[]>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected TableName[] rpcCall() throws Exception {
                List<HBaseProtos.TableName> tableNames = master.listTableNamesByNamespace(getRpcController(), ListTableNamesByNamespaceRequest.
                        newBuilder().setNamespaceName(name).build()).getTableNameList();
    ...
    public void snapshot(final String snapshotName,
            final TableName tableName) throws IOException, SnapshotCreationException, IllegalArgumentException {
    ...
    public void snapshot(final String snapshotName, final TableName tableName,
            SnapshotType type) throws IOException, SnapshotCreationException, IllegalArgumentException {
    ...
    public void snapshot(SnapshotDescription snapshotDesc) throws IOException, SnapshotCreationException, IllegalArgumentException {
        ...
        int tries = 0;
        LOG.debug("Waiting a max of " + max + " ms for snapshot '" + ClientSnapshotDescriptionUtils
                .toString(snapshot) + "'' to complete. (max " + maxPauseTime + " ms per retry)");
    ...
    @Override
    public void snapshotAsync(SnapshotDescription snapshotDesc) throws IOException, SnapshotCreationException {
        asyncSnapshot(ProtobufUtil.createHBaseProtosSnapshotDesc(snapshotDesc));
    }

    private SnapshotResponse asyncSnapshot(SnapshotProtos.SnapshotDescription snapshot) throws IOException {
    ...
    @Override
    public boolean isSnapshotFinished(final SnapshotDescription snapshotDesc) throws IOException, HBaseSnapshotException, UnknownSnapshotException {
    ...
    public void restoreSnapshot(final String snapshotName) throws IOException, RestoreSnapshotException {
        boolean takeFailSafeSnapshot = conf
                .getBoolean(HConstants.SNAPSHOT_RESTORE_TAKE_FAILSAFE_SNAPSHOT, HConstants.DEFAULT_SNAPSHOT_RESTORE_TAKE_FAILSAFE_SNAPSHOT);
        restoreSnapshot(snapshotName, takeFailSafeSnapshot);
    }

    @Override
    public void restoreSnapshot(final byte[] snapshotName, final boolean takeFailSafeSnapshot) throws IOException, RestoreSnapshotException {
     ...
     */
    private TableName getTableNameBeforeRestoreSnapshot(final String snapshotName) throws IOException, RestoreSnapshotException {
        TableName tableName = null;
    ...
    public void restoreSnapshot(final String snapshotName, final boolean takeFailSafeSnapshot,
        ...
        if(takeFailSafeSnapshot) {
            ...
            failSafeSnapshotSnapshotName = failSafeSnapshotSnapshotName.replace("{snapshot.name}", snapshotName)
        ...
        try {
            // Restore snapshot
            get(internalRestoreSnapshotAsync(snapshotName, tableName, restoreAcl), syncWaitTimeout, TimeUnit.MILLISECONDS);
        } catch(IOException e) {
            // Something went wrong during the restore...
            // if the pre-restore snapshot is available try to rollback
            if(takeFailSafeSnapshot) {
                try {
                    get(internalRestoreSnapshotAsync(failSafeSnapshotSnapshotName, tableName, restoreAcl), syncWaitTimeout, TimeUnit.MILLISECONDS);
    ...
    @Override
    public Future<Void> restoreSnapshotAsync(final String snapshotName) throws IOException, RestoreSnapshotException {
        TableName tableName = getTableNameBeforeRestoreSnapshot(snapshotName);
    ...
    public void cloneSnapshot(final byte[] snapshotName,
            final TableName tableName) throws IOException, TableExistsException, RestoreSnapshotException {
    ...
    public void cloneSnapshot(String snapshotName, TableName tableName,
        ...
        get(internalRestoreSnapshotAsync(snapshotName, tableName, restoreAcl), Integer.MAX_VALUE, TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    public Future<Void> cloneSnapshotAsync(final String snapshotName, final TableName tableName) throws IOException, TableExistsException {
        if(tableExists(tableName)) {
            throw new TableExistsException(tableName);
        }
        return internalRestoreSnapshotAsync(snapshotName, tableName, false);
    ...
    public byte[] execProcedureWithReturn(String signature, String instance, Map<String, String> props) throws IOException {
        ...
        final ExecProcedureRequest request = ExecProcedureRequest.newBuilder().setProcedure(desc).build();
    ...
    public void execProcedure(String signature, String instance, Map<String, String> props) throws IOException {
        ...
        int tries = 0;
        LOG.debug(
                "Waiting a max of " + max + " ms for procedure '" + signature + " : " + instance + "'' to complete. (max " + maxPauseTime + " ms per retry)");
        boolean done = false;
     ...
     */
    private Future<Void> internalRestoreSnapshotAsync(final String snapshotName, final TableName tableName,
            final boolean restoreAcl) throws IOException, RestoreSnapshotException {
        final SnapshotProtos.SnapshotDescription snapshot = SnapshotProtos.SnapshotDescription.newBuilder().setName(snapshotName)
        ...
        RestoreSnapshotResponse response = executeCallable(new MasterCallable<RestoreSnapshotResponse>(getConnection(), getRpcControllerFactory()) {
            ...
            protected RestoreSnapshotResponse rpcCall() throws Exception {
                final RestoreSnapshotRequest request = RestoreSnapshotRequest.newBuilder().setSnapshot(snapshot).setNonceGroup(nonceGroup)
    ...
    private static class RestoreSnapshotFuture extends TableFuture<Void> {
        public RestoreSnapshotFuture(final HBaseAdmin admin, final SnapshotProtos.SnapshotDescription snapshot, final TableName tableName,
        ...
        public RestoreSnapshotFuture(final HBaseAdmin admin, final TableName tableName, final Long procId) {
            super(admin, tableName, procId);
        }
        ...
        return executeCallable(new MasterCallable<List<SnapshotDescription>>(getConnection(), getRpcControllerFactory()) {
            ...
            protected List<SnapshotDescription> rpcCall() throws Exception {
                List<SnapshotProtos.SnapshotDescription> snapshotsList = master
                        .getCompletedSnapshots(getRpcController(), GetCompletedSnapshotsRequest.newBuilder().build()).getSnapshotsList();
        ...
        executeCallable(new MasterCallable<Void>(getConnection(), getRpcControllerFactory()) {
            ...
            protected Void rpcCall() throws Exception {
                master.deleteSnapshot(getRpcController(),
                        DeleteSnapshotRequest.newBuilder().setSnapshot(SnapshotProtos.SnapshotDescription.newBuilder().setName(snapshotName).build())
                                .build());
                return null;
            }
        });
    ...
    private void internalDeleteSnapshot(final SnapshotDescription snapshot) throws IOException {
        executeCallable(new MasterCallable<Void>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Void rpcCall() throws Exception {
                this.master.deleteSnapshot(getRpcController(),
                        DeleteSnapshotRequest.newBuilder().setSnapshot(ProtobufUtil.createHBaseProtosSnapshotDesc(snapshot)).build());
                return null;
            }
        });
    }
    ...
    static private <C extends RetryingCallable<V> & Closeable, V> V executeCallable(C callable, RpcRetryingCallerFactory rpcCallerFactory,
            int operationTimeout, int rpcTimeout) throws IOException {

        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： caller = RpcRetryingCallerImpl
         */
    ...
    public CoprocessorRpcChannel coprocessorService() {
        return new SyncCoprocessorRpcChannel() {
            @Override
            protected Message callExecService(final RpcController controller, final Descriptors.MethodDescriptor method, final Message request,
                    final Message responsePrototype) throws IOException {
                if(LOG.isTraceEnabled()) {
                    LOG.trace("Call: " + method.getName() + ", " + request.toString());
                }
                // Try-with-resources so close gets called when we are done.
                try(MasterCallable<CoprocessorServiceResponse> callable = new MasterCallable<CoprocessorServiceResponse>(connection,
    ...
    public CoprocessorRpcChannel coprocessorService(final ServerName serverName) {
        return new SyncCoprocessorRpcChannel() {
            @Override
            protected Message callExecService(RpcController controller, Descriptors.MethodDescriptor method, Message request,
    ...
    public void updateConfiguration() throws IOException {
        ClusterMetrics status = getClusterMetrics(EnumSet.of(Option.LIVE_SERVERS, Option.MASTER, Option.BACKUP_MASTERS));
    ...
    public long getLastMajorCompactionTimestampForRegion(final byte[] regionName) throws IOException {
        return executeCallable(new MasterCallable<Long>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Long rpcCall() throws Exception {
                MajorCompactionTimestampForRegionRequest req = MajorCompactionTimestampForRegionRequest.newBuilder()
     ...
     */
    @Override
    public void compact(final TableName tableName, final byte[] columnFamily, CompactType compactType) throws IOException, InterruptedException {
     ...
     */
    @Override
    public void compact(final TableName tableName, CompactType compactType) throws IOException, InterruptedException {
     ...
     */
    @Override
    public void majorCompact(final TableName tableName, final byte[] columnFamily, CompactType compactType) throws IOException, InterruptedException {
     ...
     */
    @Override
    public void majorCompact(final TableName tableName, CompactType compactType) throws IOException, InterruptedException {
            ...
            case NORMAL:
                for(HRegionLocation loc : connection.locateRegions(tableName, false, false)) {
                    ...
                    try {
                        ...
                        if(LOG.isDebugEnabled()) {
                            LOG.debug("Trying to get compaction state of " + loc.getRegion() + ": " + StringUtils.stringifyException(e));
                        }
    ...
    protected static class ProcedureFuture<V> implements Future<V> {
        ...
        protected AbortProcedureResponse abortProcedureResult(final AbortProcedureRequest request) throws IOException {
        ...
        public V get() throws InterruptedException, ExecutionException {
            ...
            try {
                ...
                LOG.warn("Failed to get the procedure with procId=" + procId + " throws exception " + e.getMessage(), e);
                return null;
            }
        }
        ...
        protected void waitForState(final long deadlineTs, final WaitForStateCallable callable) throws IOException, TimeoutException {
            int tries = 0;
    ...
    protected static abstract class TableFuture<V> extends ProcedureFuture<V> {
        ...
        protected String getDescription() {
            return "Operation: " + getOperationType() + ", " + "Table Name: " + tableName.getNameWithNamespaceInclAsString() + ", procId: " + procId;
        }
        ...
        protected abstract class TableWaitForStateCallable implements WaitForStateCallable {
            ...
            public void throwTimeoutException(long elapsedTime) throws TimeoutException {
                throw new TimeoutException(getDescription() + " has not completed after " + elapsedTime + "ms");
            }
        }
        ...
        protected void waitForTableDisabled(final long deadlineTs) throws IOException, TimeoutException {
            waitForState(deadlineTs, new TableWaitForStateCallable() {
                @Override
                public boolean checkState(int tries) throws IOException {
        ...
        protected void waitForAllRegionsOnline(final long deadlineTs, final byte[][] splitKeys) throws IOException, TimeoutException {
            ...
            while(EnvironmentEdgeManager.currentTime() < deadlineTs) {
                ...
                try {
                    ...
                    throw new InterruptedIOException(
                            "Interrupted when opening" + " regions; " + actualRegCount.get() + " of " + numRegs + " regions processed so far");
                }
            }
    ...
    private boolean splitOrMergeSwitch(boolean enabled, boolean synchronous, MasterSwitchType switchType) throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                MasterProtos.SetSplitOrMergeEnabledResponse response = master.setSplitOrMergeEnabled(getRpcController(),
    ...
    public boolean isSplitEnabled() throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                return master.isSplitOrMergeEnabled(getRpcController(), RequestConverter.buildIsSplitOrMergeEnabledRequest(MasterSwitchType.SPLIT))
                        .getEnabled();
            }
        });
    }
    ...
    public boolean isMergeEnabled() throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                return master.isSplitOrMergeEnabled(getRpcController(), RequestConverter.buildIsSplitOrMergeEnabledRequest(MasterSwitchType.MERGE))
                        .getEnabled();
            }
        });
    }
    ...
    public void addReplicationPeer(String peerId, ReplicationPeerConfig peerConfig, boolean enabled) throws IOException {
        get(addReplicationPeerAsync(peerId, peerConfig, enabled), this.syncWaitTimeout, TimeUnit.MILLISECONDS);
    }
        ...
        AddReplicationPeerResponse response = executeCallable(
                new MasterCallable<AddReplicationPeerResponse>(getConnection(), getRpcControllerFactory()) {
                    ...
                    protected AddReplicationPeerResponse rpcCall() throws Exception {
                        return master
                                .addReplicationPeer(getRpcController(), RequestConverter.buildAddReplicationPeerRequest(peerId, peerConfig, enabled));
                    }
                });
    ...
    public Future<Void> removeReplicationPeerAsync(String peerId) throws IOException {
        RemoveReplicationPeerResponse response = executeCallable(
                new MasterCallable<RemoveReplicationPeerResponse>(getConnection(), getRpcControllerFactory()) {
                    @Override
                    protected RemoveReplicationPeerResponse rpcCall() throws Exception {
                        return master.removeReplicationPeer(getRpcController(), RequestConverter.buildRemoveReplicationPeerRequest(peerId));
                    }
                });
    ...
    public Future<Void> enableReplicationPeerAsync(final String peerId) throws IOException {
        EnableReplicationPeerResponse response = executeCallable(
                new MasterCallable<EnableReplicationPeerResponse>(getConnection(), getRpcControllerFactory()) {
                    @Override
                    protected EnableReplicationPeerResponse rpcCall() throws Exception {
                        return master.enableReplicationPeer(getRpcController(), RequestConverter.buildEnableReplicationPeerRequest(peerId));
                    }
                });
    ...
    public Future<Void> disableReplicationPeerAsync(final String peerId) throws IOException {
        DisableReplicationPeerResponse response = executeCallable(
                new MasterCallable<DisableReplicationPeerResponse>(getConnection(), getRpcControllerFactory()) {
                    @Override
                    protected DisableReplicationPeerResponse rpcCall() throws Exception {
                        return master.disableReplicationPeer(getRpcController(), RequestConverter.buildDisableReplicationPeerRequest(peerId));
                    }
                });
    ...
    public ReplicationPeerConfig getReplicationPeerConfig(final String peerId) throws IOException {
        return executeCallable(new MasterCallable<ReplicationPeerConfig>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected ReplicationPeerConfig rpcCall() throws Exception {
                GetReplicationPeerConfigResponse response = master
                        .getReplicationPeerConfig(getRpcController(), RequestConverter.buildGetReplicationPeerConfigRequest(peerId));
    ...
    @Override
    public void updateReplicationPeerConfig(final String peerId, final ReplicationPeerConfig peerConfig) throws IOException {
        get(updateReplicationPeerConfigAsync(peerId, peerConfig), this.syncWaitTimeout, TimeUnit.MILLISECONDS);
    }
    ...
    public Future<Void> updateReplicationPeerConfigAsync(final String peerId, final ReplicationPeerConfig peerConfig) throws IOException {
        UpdateReplicationPeerConfigResponse response = executeCallable(
                new MasterCallable<UpdateReplicationPeerConfigResponse>(getConnection(), getRpcControllerFactory()) {
                    @Override
    ...
    public List<ReplicationPeerDescription> listReplicationPeers(Pattern pattern) throws IOException {
        return executeCallable(new MasterCallable<List<ReplicationPeerDescription>>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected List<ReplicationPeerDescription> rpcCall() throws Exception {
                List<ReplicationProtos.ReplicationPeerDescription> peersList = master
                        .listReplicationPeers(getRpcController(), RequestConverter.buildListReplicationPeersRequest(pattern)).getPeerDescList();
    ...
    public void decommissionRegionServers(List<ServerName> servers, boolean offload) throws IOException {
        executeCallable(new MasterCallable<Void>(getConnection(), getRpcControllerFactory()) {
            ...
            public Void rpcCall() throws ServiceException {
                master.decommissionRegionServers(getRpcController(), RequestConverter.buildDecommissionRegionServersRequest(servers, offload));
                return null;
            }
        });
    }
    ...
    public List<ServerName> listDecommissionedRegionServers() throws IOException {
        return executeCallable(new MasterCallable<List<ServerName>>(getConnection(), getRpcControllerFactory()) {
            @Override
            public List<ServerName> rpcCall() throws ServiceException {
                ...
                for(HBaseProtos.ServerName server : master.listDecommissionedRegionServers(getRpcController(), req).getServerNameList()) {
    ...
    private void checkAndSyncTableDescToPeers(final TableName tableName, final byte[][] splits) throws IOException {
        ...
        for(ReplicationPeerDescription peerDesc : peers) {
            if(peerDesc.getPeerConfig().needToReplicate(tableName)) {
                ...
                try(Connection conn = ConnectionFactory.createConnection(peerConf); Admin repHBaseAdmin = conn.getAdmin()) {
                    ...
                    if(!repHBaseAdmin.tableExists(tableName)) {
                        ...
                        if(peerTableDesc == null) {
                            throw new IllegalArgumentException(
                                    "Failed to get table descriptor for table " + tableName.getNameAsString() + " from peer cluster " + peerDesc
                                            .getPeerId());
                        }
                        if(TableDescriptor.COMPARATOR_IGNORE_REPLICATION.compare(peerTableDesc, tableDesc) != 0) {
                            throw new IllegalArgumentException("Table " + tableName.getNameAsString() + " exists in peer cluster " + peerDesc
                                    .getPeerId() + ", but the table descriptors are not same when compared with source cluster." + " Thus can not enable the table's replication switch.");
                        }
                    }
                }
            }
    ...
    @Override
    public void clearCompactionQueues(final ServerName sn, final Set<String> queues) throws IOException, InterruptedException {
        ...
        Callable<Void> callable = new Callable<Void>() {
            ...
            public Void call() throws Exception {
                // TODO: There is no timeout on this controller. Set one!
                HBaseRpcController controller = rpcControllerFactory.newController();
                ClearCompactionQueuesRequest request = RequestConverter.buildClearCompactionQueuesRequest(queues);
    ...
    public List<ServerName> clearDeadServers(List<ServerName> servers) throws IOException {
        return executeCallable(new MasterCallable<List<ServerName>>(getConnection(), getRpcControllerFactory()) {
            ...
            protected List<ServerName> rpcCall() throws Exception {
                ...
                return ProtobufUtil.toServerNameList(master.clearDeadServers(getRpcController(), req).getServerNameList());
            }
        });
    }

    @Override
    public void cloneTableSchema(final TableName tableName, final TableName newTableName, final boolean preserveSplits) throws IOException {
        checkTableExists(tableName);
        ...
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            ...
            protected Boolean rpcCall() throws Exception {
                return this.master.switchRpcThrottle(getRpcController(),
                        MasterProtos.SwitchRpcThrottleRequest.newBuilder().setRpcThrottleEnabled(enable).build()).getPreviousRpcThrottleEnabled();
            }
        });
    }
    ...
    public boolean isRpcThrottleEnabled() throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                return this.master.isRpcThrottleEnabled(getRpcController(), IsRpcThrottleEnabledRequest.newBuilder().build()).getRpcThrottleEnabled();
            }
        });
    }
    ...
    public boolean exceedThrottleQuotaSwitch(final boolean enable) throws IOException {
        return executeCallable(new MasterCallable<Boolean>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Boolean rpcCall() throws Exception {
                return this.master.switchExceedThrottleQuota(getRpcController(),
                        MasterProtos.SwitchExceedThrottleQuotaRequest.newBuilder().setExceedThrottleQuotaEnabled(enable).build())
    ...
    public Map<TableName, Long> getSpaceQuotaTableSizes() throws IOException {
        return executeCallable(new MasterCallable<Map<TableName, Long>>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Map<TableName, Long> rpcCall() throws Exception {
                GetSpaceQuotaRegionSizesResponse resp = master
                        .getSpaceQuotaRegionSizes(getRpcController(), RequestConverter.buildGetSpaceQuotaRegionSizesRequest());
    ...
    public Map<TableName, SpaceQuotaSnapshot> getRegionServerSpaceQuotaSnapshots(ServerName serverName) throws IOException {
        ...
        Callable<GetSpaceQuotaSnapshotsResponse> callable = new Callable<GetSpaceQuotaSnapshotsResponse>() {
            @Override
            public GetSpaceQuotaSnapshotsResponse call() throws Exception {
                return admin.getSpaceQuotaSnapshots(rpcControllerFactory.newController(), RequestConverter.buildGetSpaceQuotaSnapshotsRequest());
            }
        };
    ...
    public SpaceQuotaSnapshot getCurrentSpaceQuotaSnapshot(String namespace) throws IOException {
        return executeCallable(new MasterCallable<SpaceQuotaSnapshot>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected SpaceQuotaSnapshot rpcCall() throws Exception {
                GetQuotaStatesResponse resp = master.getQuotaStates(getRpcController(), RequestConverter.buildGetQuotaStatesRequest());
    ...
    public void grant(UserPermission userPermission, boolean mergeExistingPermissions) throws IOException {
        executeCallable(new MasterCallable<Void>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected Void rpcCall() throws Exception {
                GrantRequest req = ShadedAccessControlUtil.buildGrantRequest(userPermission, mergeExistingPermissions);
    ...
    public List<UserPermission> getUserPermissions(GetUserPermissionsRequest getUserPermissionsRequest) throws IOException {
        return executeCallable(new MasterCallable<List<UserPermission>>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected List<UserPermission> rpcCall() throws Exception {
                ...
                AccessControlProtos.GetUserPermissionsResponse response = this.master.getUserPermissions(getRpcController(), req);
                return response.getUserPermissionList().stream().map(userPermission -> ShadedAccessControlUtil.toUserPermission(userPermission))
                        .collect(Collectors.toList());
            }
        });
    }
    ...
    public List<Boolean> hasUserPermissions(String userName, List<Permission> permissions) throws IOException {
        return executeCallable(new MasterCallable<List<Boolean>>(getConnection(), getRpcControllerFactory()) {
            @Override
            protected List<Boolean> rpcCall() throws Exception {
                HasUserPermissionsRequest request = ShadedAccessControlUtil.buildHasUserPermissionsRequest(userName, permissions);
                return this.master.hasUserPermissions(getRpcController(), request).getHasUserPermissionList();
            }
        });
    }
}
