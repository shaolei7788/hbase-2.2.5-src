...
public class HRegionLocator implements RegionLocator {
    ...
    public HRegionLocation getRegionLocation(byte[] row, int replicaId, boolean reload) throws IOException {
         ...
         *   注释： 定位 Region
         */
        return connection.locateRegion(tableName, row, !reload, true, replicaId).getRegionLocation(replicaId);
    }

    @Override
    public List<HRegionLocation> getRegionLocations(byte[] row, boolean reload) throws IOException {
        RegionLocations locs = connection.locateRegion(tableName, row, !reload, true, RegionInfo.DEFAULT_REPLICA_ID);
    ...
    private List<RegionLocations> listRegionLocations() throws IOException {
        if(TableName.isMetaTableName(tableName)) {
            return Collections.singletonList(connection.locateRegion(tableName, HConstants.EMPTY_START_ROW, false, true));
        }
