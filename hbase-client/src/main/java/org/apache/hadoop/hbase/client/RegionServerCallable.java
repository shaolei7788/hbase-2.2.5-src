...
public abstract class RegionServerCallable<T, S> implements RetryingCallable<T> {
    ...
    public RegionServerCallable(Connection connection, TableName tableName, byte[] row, RpcController rpcController, int priority) {
        super();
         ...
         *   注释： 你的 get 或者 put 请求的 rowkey
         *   两个大作用：
         *   1、用来确定该 rowkey 到底处于哪个 region 中
         *   2、寻找 key-value 真实数据的依据
         */
        this.row = row;
    ...
    protected CellScanner getRpcControllerCellScanner() {
        return (getRpcController() != null && getRpcController() instanceof HBaseRpcController) ? ((HBaseRpcController) getRpcController())
                .cellScanner() : null;
    }
    ...
    public void throwable(Throwable t, boolean retrying) {
        if(location != null) {
             ...
             *   注释：
             */
            getConnection().updateCachedLocations(tableName, location.getRegionInfo().getRegionName(), row, t, location.getServerName());
        }
    }
    ...
    public void prepare(final boolean reload) throws IOException {
        ...
        // TODO_MA 注释：构建一个 HRegionLocator 对象！
        try(RegionLocator regionLocator = connection.getRegionLocator(tableName)) {

            /********
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *   注释： 这个 row 就是 Callble 在初始化的时候拿到的 rowkey
