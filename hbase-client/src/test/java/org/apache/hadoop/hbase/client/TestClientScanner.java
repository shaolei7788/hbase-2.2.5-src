...
public class TestClientScanner {
    ...
    private static class MockClientScanner extends ClientSimpleScanner {
        ...
        public MockClientScanner(final Configuration conf, final Scan scan, final TableName tableName, ClusterConnection connection,
    ...
    public void testNoResultsHint() throws IOException {
        ...
        try(MockClientScanner scanner = new MockClientScanner(conf, scan, TableName.valueOf(name.getMethodName()), clusterConn, rpcFactory,
                controllerFactory, pool, Integer.MAX_VALUE)) {
    ...
    public void testExceptionsFromReplicasArePropagated() throws IOException {
        ...
        when(clusterConn.locateRegion((TableName) any(), (byte[]) any(), anyBoolean(), anyBoolean(), anyInt()))
                .thenReturn(new RegionLocations(null, null, null));

        try(MockClientScanner scanner = new MockClientScanner(conf, scan, TableName.valueOf(name.getMethodName()), clusterConn, rpcFactory,
                new RpcControllerFactory(conf), pool, Integer.MAX_VALUE)) {
