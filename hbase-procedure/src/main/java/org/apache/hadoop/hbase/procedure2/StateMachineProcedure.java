...
public abstract class StateMachineProcedure<TEnvironment, TState> extends Procedure<TEnvironment> {
     ...
     */
    protected abstract Flow executeFromState(TEnvironment env,
            TState state) throws ProcedureSuspendedException, ProcedureYieldException, InterruptedException;
     ...
     */
    protected <T extends Procedure<TEnvironment>> void addChildProcedure(@SuppressWarnings("unchecked") T... subProcedure) {
     ...
     */
    @Override
    protected Procedure[] execute(final TEnvironment env) throws ProcedureSuspendedException, ProcedureYieldException, InterruptedException {
        updateTimestamp();
        try {
            failIfAborted();
            ...
            // TODO_MA 注释：SPLIT_TABLE_REGION_PREPARE
