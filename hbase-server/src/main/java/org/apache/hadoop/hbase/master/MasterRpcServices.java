...
public class MasterRpcServices extends RSRpcServices implements MasterService.BlockingInterface, RegionServerStatusService.BlockingInterface, LockService.BlockingInterface, HbckService.BlockingInterface {
    ...
    protected RpcServerInterface createRpcServer(Server server, Configuration conf, RpcSchedulerFactory rpcSchedulerFactory,
        ...
        try {
            ...
            throw new IOException(be.getMessage() + ". To switch ports use the '" + HConstants.MASTER_PORT + "' configuration property.",
    ...
    public RegionServerStartupResponse regionServerStartup(RpcController controller, RegionServerStartupRequest request) throws ServiceException {
        ...
        try {
             ...
             *   注释：
             */
            // if regionserver passed hostname to use, then use it instead of doing a reverse DNS lookup
    ...
    public AddColumnResponse addColumn(RpcController controller, AddColumnRequest req) throws ServiceException {
        try {
            long procId = master
                    .addColumn(ProtobufUtil.toTableName(req.getTableName()), ProtobufUtil.toColumnFamilyDescriptor(req.getColumnFamilies()),
    ...
    public CreateNamespaceResponse createNamespace(RpcController controller, CreateNamespaceRequest request) throws ServiceException {
        try {
            long procId = master.createNamespace(ProtobufUtil.toNamespaceDescriptor(request.getNamespaceDescriptor()), request.getNonceGroup(),
                    request.getNonce());
    ...
    public CreateTableResponse createTable(RpcController controller, CreateTableRequest req) throws ServiceException {
        ...
        try {
            ...
            LOG.info(master.getClientIdAuditPrefix() + " procedure request for creating table: " + req.getTableSchema()
                    .getTableName() + " procId is: " + procId);
    ...
    public DeleteColumnResponse deleteColumn(RpcController controller, DeleteColumnRequest req) throws ServiceException {
        try {
            long procId = master.deleteColumn(ProtobufUtil.toTableName(req.getTableName()), req.getColumnName().toByteArray(), req.getNonceGroup(),
                    req.getNonce());
            if(procId == -1) {
                // This mean operation was not performed in server, so do not set any procId
    ...
    public TruncateTableResponse truncateTable(RpcController controller, TruncateTableRequest request) throws ServiceException {
        try {
            long procId = master.truncateTable(ProtobufUtil.toTableName(request.getTableName()), request.getPreserveSplits(), request.getNonceGroup(),
                    request.getNonce());
    ...
    public MergeTableRegionsResponse mergeTableRegions(RpcController c, MergeTableRegionsRequest request) throws ServiceException {
        ...
        for(int i = 0; i < request.getRegionCount(); i++) {
            ...
            if(request.getRegion(i).getType() != RegionSpecifierType.ENCODED_REGION_NAME) {
                LOG.warn("MergeRegions specifier type: expected: " + RegionSpecifierType.ENCODED_REGION_NAME + " actual: region " + i + " =" + request
                        .getRegion(i).getType());
            }
    ...
    public SplitTableRegionResponse splitRegion(final RpcController controller, final SplitTableRegionRequest request) throws ServiceException {
        try {
            long procId = master.splitRegion(ProtobufUtil.toRegionInfo(request.getRegionInfo()),
                    request.hasSplitRow() ? request.getSplitRow().toByteArray() : null, request.getNonceGroup(), request.getNonce());
        ...
        try {
            ...
            if(!master.coprocessorServiceHandlers.containsKey(serviceName)) {
                throw new UnknownProtocolException(null,
                        "No registered Master Coprocessor Endpoint found for " + serviceName + ". Has it been enabled?");
            }
    ...
    public GetClusterStatusResponse getClusterStatus(RpcController controller, GetClusterStatusRequest req) throws ServiceException {
        ...
        try {
            ...
            response.setClusterStatus(
                    ClusterMetricsBuilder.toClusterStatus(master.getClusterMetrics(ClusterMetricsBuilder.toOptions(req.getOptionsList()))));
        } catch(IOException e) {
            throw new ServiceException(e);
        }
    ...
    public GetNamespaceDescriptorResponse getNamespaceDescriptor(RpcController controller,
        ...
        try {
            return GetNamespaceDescriptorResponse.newBuilder()
                    .setNamespaceDescriptor(ProtobufUtil.toProtoNamespaceDescriptor(master.getNamespace(request.getNamespaceName()))).build();
        } catch(IOException e) {
            throw new ServiceException(e);
        }
    }
    ...
    public ModifyColumnResponse modifyColumn(RpcController controller, ModifyColumnRequest req) throws ServiceException {
        try {
            long procId = master
                    .modifyColumn(ProtobufUtil.toTableName(req.getTableName()), ProtobufUtil.toColumnFamilyDescriptor(req.getColumnFamilies()),
                            req.getNonceGroup(), req.getNonce());
            if(procId == -1) {
                // This mean operation was not performed in server, so do not set any procId
    ...
    public ModifyNamespaceResponse modifyNamespace(RpcController controller, ModifyNamespaceRequest request) throws ServiceException {
        try {
            long procId = master.modifyNamespace(ProtobufUtil.toNamespaceDescriptor(request.getNamespaceDescriptor()), request.getNonceGroup(),
                    request.getNonce());
    ...
    public ModifyTableResponse modifyTable(RpcController controller, ModifyTableRequest req) throws ServiceException {
        try {
            long procId = master.modifyTable(ProtobufUtil.toTableName(req.getTableName()), ProtobufUtil.toTableDescriptor(req.getTableSchema()),
                    req.getNonceGroup(), req.getNonce());
    ...
    public MoveRegionResponse moveRegion(RpcController controller, MoveRegionRequest req) throws ServiceException {
        ...
        final byte[] destServerName = (req.hasDestServerName()) ? Bytes
                .toBytes(ProtobufUtil.toServerName(req.getDestServerName()).getServerName()) : null;
    ...
    public SnapshotResponse snapshot(RpcController controller, SnapshotRequest request) throws ServiceException {
        try {
            ...
            long waitTime = SnapshotDescriptionUtils
                    .getMaxMasterTimeout(master.getConfiguration(), snapshot.getType(), SnapshotDescriptionUtils.DEFAULT_MAX_WAIT_TIME);
    ...
    public UnassignRegionResponse unassignRegion(RpcController controller, UnassignRegionRequest req) throws ServiceException {
        try {
            ...
            if(Bytes.equals(RegionInfoBuilder.FIRST_META_REGIONINFO.getRegionName(), regionName)) {
                pair = new Pair<>(RegionInfoBuilder.FIRST_META_REGIONINFO, MetaTableLocator.getMetaRegionLocation(master.getZooKeeper()));
            }
            if(pair == null) {
            ...
            LOG.debug(master.getClientIdAuditPrefix() + " unassign " + hri
                    .getRegionNameAsString() + " in current location if it is online and reassign.force=" + force);
    ...
    private void checkHFileFormatVersionForMob() throws IOException {
        if(HFile.getFormatVersion(master.getConfiguration()) < HFile.MIN_FORMAT_VERSION_WITH_TAGS) {
            LOG.error(
                    "A minimum HFile version of " + HFile.MIN_FORMAT_VERSION_WITH_TAGS + " is required for MOB compaction. Compaction will not run.");
    ...
    public AddReplicationPeerResponse addReplicationPeer(RpcController controller, AddReplicationPeerRequest request) throws ServiceException {
        try {
            long procId = master.addReplicationPeer(request.getPeerId(), ReplicationPeerConfigUtil.convert(request.getPeerConfig()),
    ...
    public DecommissionRegionServersResponse decommissionRegionServers(RpcController controller,
        ...
        try {
            master.checkInitialized();
            List<ServerName> servers = request.getServerNameList().stream().map(pbServer -> ProtobufUtil.toServerName(pbServer))
    ...
    public RecommissionRegionServerResponse recommissionRegionServer(RpcController controller,
        ...
        try {
            ...
            List<byte[]> encodedRegionNames = request.getRegionList().stream().map(regionSpecifier -> regionSpecifier.getValue().toByteArray())
                    .collect(Collectors.toList());
            if(master.cpHost != null) {
        ...
        try {
            ...
            if(request.getRegionInfoCount() > 0) {
                ...
                npr = new NonceProcedureRunnable(master, request.getNonceGroup(), request.getNonce()) {
                    @Override
                    protected void run() throws IOException {
                        setProcId(master.getLockManager().remoteLocks()
                                .requestNamespaceLock(request.getNamespace(), type, request.getDescription(), getNonceKey()));
                    }
        ...
        try {
            if(master.getLockManager().remoteLocks().lockHeartbeat(request.getProcId(), request.getKeepAlive())) {
                return LockHeartbeatResponse.newBuilder().setTimeoutMs(
                        master.getConfiguration().getInt(LockProcedure.REMOTE_LOCKS_TIMEOUT_MS_CONF, LockProcedure.DEFAULT_REMOTE_LOCKS_TIMEOUT_MS))
        ...
        try {
            ...
            if(quotaManager != null) {
                ...
                for(Entry<TableName, Long> tableSize : regionSizesByTable.entrySet()) {
                    builder.addSizes(
                            RegionSizes.newBuilder().setTableName(ProtobufUtil.toProtoTableName(tableSize.getKey())).setSize(tableSize.getValue())
                                    .build());
                }
                return builder.build();
            } else {
        ...
        try {
            ...
            if(quotaChore != null) {
                ...
                for(Entry<TableName, SpaceQuotaSnapshot> entry : tableSnapshots.entrySet()) {
                    builder.addTableSnapshots(TableQuotaSnapshot.newBuilder().setTableName(ProtobufUtil.toProtoTableName(entry.getKey()))
    ...
    public ClearDeadServersResponse clearDeadServers(RpcController controller, ClearDeadServersRequest request) throws ServiceException {
        ...
        try {
            ...
            if(master.cpHost != null) {
                master.cpHost.postClearDeadServers(ProtobufUtil.toServerNameList(request.getServerNameList()),
    ...
    public MasterProtos.AssignsResponse assigns(RpcController controller, MasterProtos.AssignsRequest request) throws ServiceException {
        ...
        try {
            ...
            for(HBaseProtos.RegionSpecifier rs : request.getRegionList()) {
                ...
                responseBuilder.addPid(this.master.getMasterProcedureExecutor()
                        .submitProcedure(this.master.getAssignmentManager().createOneAssignProcedure(ri, override)));
            }
    ...
    public MasterProtos.UnassignsResponse unassigns(RpcController controller, MasterProtos.UnassignsRequest request) throws ServiceException {
        ...
        try {
            ...
            for(HBaseProtos.RegionSpecifier rs : request.getRegionList()) {
                ...
                responseBuilder.addPid(this.master.getMasterProcedureExecutor()
                        .submitProcedure(this.master.getAssignmentManager().createOneUnassignProcedure(ri, override)));
            }
            return responseBuilder.build();
        } catch(IOException ioe) {
            throw new ServiceException(ioe);
    ...
    public MasterProtos.BypassProcedureResponse bypassProcedure(RpcController controller,
        ...
        try {
            LOG.info("{} bypass procedures={}, waitTime={}, override={}, recursive={}", master.getClientIdAuditPrefix(), request.getProcIdList(),
                    request.getWaitTime(), request.getOverride(), request.getRecursive());
            List<Boolean> ret = master.getMasterProcedureExecutor()
                    .bypassProcedure(request.getProcIdList(), request.getWaitTime(), request.getOverride(), request.getRecursive());
    ...
    @Override
    public MasterProtos.ScheduleServerCrashProcedureResponse scheduleServerCrashProcedure(RpcController controller,
            MasterProtos.ScheduleServerCrashProcedureRequest request) throws ServiceException {
        ...
        try {
            ...
            if(master.cpHost != null && hasAccessControlServiceCoprocessor(master.cpHost)) {
                ...
                try(Table table = master.getConnection().getTable(AccessControlLists.ACL_TABLE_NAME)) {
                    AccessControlLists.addUserPermission(getConfiguration(), perm, table, mergeExistingPermissions);
                }
        ...
        try {
            ...
            if(master.cpHost != null && hasAccessControlServiceCoprocessor(master.cpHost)) {
                ...
                if(permissionType == Type.Table) {
                    ...
                    perms = AccessControlLists.getUserTablePermissions(master.getConfiguration(), table, cf, cq, userName, filter);
                } else if(permissionType == Type.Namespace) {
                    perms = AccessControlLists
                            .getUserNamespacePermissions(master.getConfiguration(), namespace, userName, userName != null ? true : false);
                } else {
                    perms = AccessControlLists
                            .getUserPermissions(master.getConfiguration(), null, null, null, userName, userName != null ? true : false);
            ...
            if(master.cpHost != null && hasAccessControlServiceCoprocessor(master.cpHost)) {
                ...
                if(accessChecker != null) {
                    for(Permission permission : permissions) {
                        boolean hasUserPermission = accessChecker.hasUserPermission(caller, "hasUserPermissions", permission);
