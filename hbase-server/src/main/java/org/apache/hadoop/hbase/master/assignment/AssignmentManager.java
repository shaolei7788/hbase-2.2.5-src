...
public class AssignmentManager {
    ...
    public void start() throws IOException, KeeperException {
        ...
        if(zkw != null) {
            ...
            RegionStateNode regionNode = regionStates.getOrCreateRegionStateNode(RegionInfoBuilder.FIRST_META_REGIONINFO);
            regionNode.lock();
            try {
    ...
    public List<ServerName> getFavoredNodes(final RegionInfo regionInfo) {
        return this.shouldAssignRegionsWithFavoredNodes ? ((FavoredStochasticBalancer) getBalancer())
                .getFavoredNodes(regionInfo) : ServerName.EMPTY_SERVER_LIST;
    }
    ...
    public TransitRegionStateProcedure[] createRoundRobinAssignProcedures(List<RegionInfo> hris, List<ServerName> serversToExclude) {
        ...
        try {
             ...
             *   注释： getBalancer() = StochasticLoadBalancer
             */
            // Ask the balancer to assign our regions. Pass the regions en masse.
            // The balancer can do a better job if it has all the assignments in the one lump.
            Map<ServerName, List<RegionInfo>> assignments = getBalancer()
                    .roundRobinAssignment(hris, this.master.getServerManager().createDestinationServersList(serversToExclude));
    ...
    private TransitRegionStateProcedure createAssignProcedure(RegionStateNode regionNode, ServerName targetServer, boolean override) {
        ...
        try {
            ...
            proc = TransitRegionStateProcedure.assign(getProcedureEnvironment(), regionNode.getRegionInfo(), targetServer);
            regionNode.setProcedure(proc);
        } finally {
            regionNode.unlock();
        }
    ...
    private TransitRegionStateProcedure createUnassignProcedure(RegionStateNode regionNode, boolean override) {
        ...
        try {
            ...
            assert regionNode.getProcedure() == null;
            proc = TransitRegionStateProcedure.unassign(getProcedureEnvironment(), regionNode.getRegionInfo());
            regionNode.setProcedure(proc);
        } finally {
            regionNode.unlock();
        }
    ...
    public TransitRegionStateProcedure[] createAssignProcedures(List<RegionInfo> hris) {
        return hris.stream().map(hri -> regionStates.getOrCreateRegionStateNode(hri))
                .map(regionNode -> createAssignProcedure(regionNode, null, false)).sorted(AssignmentManager::compare)
    ...
    private TransitRegionStateProcedure[] createAssignProcedures(Map<ServerName, List<RegionInfo>> assignments) {
        return assignments.entrySet().stream().flatMap(e -> e.getValue().stream().map(hri -> regionStates.getOrCreateRegionStateNode(hri))
                .map(regionNode -> createAssignProcedure(regionNode, e.getKey(), false))).sorted(AssignmentManager::compare)
                .toArray(TransitRegionStateProcedure[]::new);
    }
    ...
    public TransitRegionStateProcedure[] createUnassignProceduresForDisabling(TableName tableName) {
        return regionStates.getTableRegionStateNodes(tableName).stream().map(regionNode -> {
            regionNode.lock();
            try {
                if(!regionStates.include(regionNode, false) || regionStates.isRegionOffline(regionNode.getRegionInfo())) {
                    return null;
                }
                ...
                TransitRegionStateProcedure proc = TransitRegionStateProcedure.unassign(getProcedureEnvironment(), regionNode.getRegionInfo());
                regionNode.setProcedure(proc);
                return proc;
            } finally {
                regionNode.unlock();
    ...
    // ============================================================================================
    private void reportRegionStateTransition(ReportRegionStateTransitionResponse.Builder builder, ServerName serverName,
            List<RegionStateTransition> transitionList) throws IOException {
    ...
    private void updateRegionTransition(ServerName serverName, TransitionCode state, RegionInfo regionInfo, long seqId,
        ...
        if(regionNode == null) {
            ...
            throw new UnexpectedStateException(
                    String.format("Server %s was trying to transition region %s to %s. but the region was removed.", serverName, regionInfo, state));
        }
    ...
    private boolean reportTransition(RegionStateNode regionNode, ServerStateNode serverNode, TransitionCode state, long seqId,
            long procId) throws IOException {
    ...
    private void updateRegionSplitTransition(final ServerName serverName, final TransitionCode state, final RegionInfo parent, final RegionInfo hriA,
            final RegionInfo hriB) throws IOException {

        // TODO_MA 注释： 检查 meta 表是否在线
        checkMetaLoaded(parent);

        // TODO_MA 注释：检查region的状态是否适合 split
        if(state != TransitionCode.READY_TO_SPLIT) {
            throw new UnexpectedStateException(
                    "unsupported split regionState=" + state + " for parent region " + parent + " maybe an old RS (< 2.0) had the operation in progress");
        }

        // TODO_MA 注释：检查 splitA 的 endKey 和 splitB 的 startKey 如果不一致，则停止 split
        ...
        // TODO_MA 注释： 如果 master 不支持 split 或者 merge，则停止 split
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 正式提交执行 SplitTableRegionProcedure
         */
    ...
    private void updateRegionMergeTransition(final ServerName serverName, final TransitionCode state, final RegionInfo merged, final RegionInfo hriA,
            final RegionInfo hriB) throws IOException {
        checkMetaLoaded(merged);

        if(state != TransitionCode.READY_TO_MERGE) {
            throw new UnexpectedStateException(
                    "Unsupported merge regionState=" + state + " for regionA=" + hriA + " regionB=" + hriB + " merged=" + merged + " maybe an old RS (< 2.0) had the operation in progress");
        }
        ...
        if(master.getServerManager().getVersionNumber(serverName) < 0x0200000) {
            throw new UnsupportedOperationException(
                    String.format("Merge not handled yet: regionState=%s merged=%s hriA=%s hriB=%s", state, merged, hriA, hriB));
        }
    }
    ...
    public void reportOnlineRegions(ServerName serverName, Set<byte[]> regionNames) {
        ...
        if(LOG.isTraceEnabled()) {
            LOG.trace("ReportOnlineRegions {} regionCount={}, metaLoaded={} {}", serverName, regionNames.size(), isMetaLoaded(),
    ...
    private void checkOnlineRegionsReport(ServerStateNode serverNode, Set<byte[]> regionNames) {
        ...
        for(byte[] regionName : regionNames) {
            ...
            if(regionNode == null) {
                LOG.warn("No region state node for {}, it should already be on {}", Bytes.toStringBinary(regionName), serverName);
                continue;
            }
            regionNode.lock();
            try {
                ...
                if(regionNode.isInState(State.OPENING, State.OPEN)) {
                    ...
                    if(!regionNode.getRegionLocation().equals(serverName) && diff > 1000) {
                        LOG.warn("{} reported OPEN on server={} but state has otherwise", regionNode, serverName);
                    }
                    ...
                    if(diff > 1000) {
                        LOG.warn("{} reported an unexpected OPEN on {}; time since last update={}ms", regionNode, serverName, diff);
                    }
                }
            } finally {
                regionNode.unlock();
    ...
    public void joinCluster() throws IOException {
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 加载元数据， 从磁盘加载meta表的数据到内存！同时启用这张表
         */
        // Scan hbase:meta to build list of existing regions, servers, and assignment.
        // hbase:meta is online now or will be. Inside loadMeta, we keep trying. Can't make progress w/o  meta.
        loadMeta();
    ...
    public void processOfflineRegions() {
        List<RegionInfo> offlineRegions = regionStates.getRegionStates().stream().filter(RegionState::isOffline)
                .filter(s -> isTableEnabled(s.getRegion().getTable())).map(RegionState::getRegion).collect(Collectors.toList());
    ...
    private void loadMeta() throws IOException {
        ...
        regionStateStore.visitMeta(new RegionStateStore.RegionStateVisitor() {
             ...
             */
            @Override
            public void visitRegionState(Result result, final RegionInfo regionInfo, final State state, final ServerName regionLocation,
    ...
    public void markRegionAsSplit(final RegionInfo parent, final ServerName serverName, final RegionInfo daughterA,
            final RegionInfo daughterB) throws IOException {
    ...
    private void processAssignQueue() {
        ...
        if(!systemHRIs.isEmpty()) {
            ...
            processAssignmentPlans(regions, null, systemHRIs,
                    serversForSysTables.isEmpty() && !containsBogusAssignments(regions, systemHRIs) ? servers : serversForSysTables);
        }
    ...
    private void processAssignmentPlans(final HashMap<RegionInfo, RegionStateNode> regions, final HashMap<RegionInfo, ServerName> retainMap,
            final List<RegionInfo> hris, final List<ServerName> servers) {
    ...
    public List<ServerName> getExcludedServersForSystemTable() {
        ...
        List<Pair<ServerName, String>> serverList = master.getServerManager().getOnlineServersList().stream()
    ...
    public RegionStatesCount getRegionStatesCount(TableName tableName) {
        ...
        return new RegionStatesCount.RegionStatesCountBuilder().setOpenRegions(openRegionsCount).setClosedRegions(closedRegionCount)
                .setSplitRegions(splitRegionCount).setRegionsInTransition(ritCount).setTotalRegions(totalRegionCount).build();
    }
