...
public abstract class ZKNodeTracker extends ZKListener {
     ...
     *
     * @param timeout maximum time to wait for the node data to be available, n milliseconds. Pass 0 for no timeout.
     * @return data of the node
     * @throws InterruptedException if the waiting thread is interrupted
     */
    public synchronized byte[] blockUntilAvailable(long timeout, boolean refresh) throws InterruptedException {
        ...
        // TODO_MA 注释： refresh 参数为 false， 不走这儿
        if(refresh) {
            try {
        ...
        // TODO_MA 注释：nodeExistsChecked = true
        ...
        while(!this.stopped && (notimeout || remaining > 0) && this.data == null) {
            if(!nodeExistsChecked) {
                try {
                    ...
                    LOG.warn("Got exception while trying to check existence in  ZooKeeper" + " of the node: " + node + ", retrying if timeout not reached",
                            e);
                }
