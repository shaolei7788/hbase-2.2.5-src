...
public class ZKWatcher implements Watcher, Abortable, Closeable {
     ...
     *                                      ZNodes
     */
    public ZKWatcher(Configuration conf, String identifier, Abortable abortable, boolean canCreateBaseZNode,
            boolean clientZK) throws IOException, ZooKeeperConnectionException {
        this.conf = conf;
        if(clientZK) {
        ...
        /********
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 初始化 HBase 在 zookeeper 中的 znode 路径
         */
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *   注释： 监听器
         *   创建 PendingWatcher 的时候，初始化一个 CountDownLatch
         */
         ...
         *   HMaster HRegionserver 都是通过 recoverableZooKeeper 的 zk 实例来完成操作 zookeeper
         */
        this.recoverableZooKeeper = ZKUtil.connect(conf, quorum, pendingWatcher, identifier);
        // TODO_MA 注释： 释放 CountDownLatch
    ...
    private void createBaseZNodes() throws ZooKeeperConnectionException {
        try {
             ...
             *   1、baseZNode = /hbase
             *   2、rsZNode = /hbase/rs
             *   3、drainingZNode = /hbase/draining
             *   4、tableZNode = /hbase/table
             *   5、splitLogZNode= /hbase/splitWAL
             *   6、backupMasterAddressesZNode = /hbase/backup-masters
             *   7、masterMaintZNode = /hbase/master-maintenance
             */
                ...
                for(ACL acl : acls) {
                    if(user.equals(acl.getId().getId())) {
                        if(acl.getPerms() == Perms.ALL) {
                            ...
                            if(LOG.isDebugEnabled()) {
                                LOG.debug(String.format("superuser '%s' does not have correct permissions: have 0x%x, want 0x%x", acl.getId().getId(),
                                        acl.getPerms(), Perms.ALL));
                            }
                        }
                        break;
                    }
    ...
    public void process(WatchedEvent event) {
        LOG.debug(prefix("Received ZooKeeper Event, " + "type=" + event.getType() + ", " + "state=" + event.getState() + ", " + "path=" + event
                .getPath()));
    ...
    public void interruptedExceptionNoThrow(InterruptedException ie, boolean throwLater) {
        LOG.debug(prefix("Received InterruptedException, will interrupt current thread" + (throwLater ? " and rethrow a SystemErrorException" : "")),
                ie);
